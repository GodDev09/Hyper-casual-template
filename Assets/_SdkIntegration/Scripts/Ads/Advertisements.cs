...
namespace ATSoft.Ads
{
    public class Advertisements : Singleton<Advertisements>, IService
    {
        ...
        [OnValueChanged("ChangeTypeAdvertiserHandle")] [SerializeField] private SupportedAdvertisers typeAdvertiser = SupportedAdvertisers.None;
        [OnValueChanged("ChangePublisherHandle")] [SerializeField] private ATPublisher publisher = ATPublisher.InHouse;
...
// #endif
        ...
        private void ContinueInitialization()
        {
            switch (typeAdvertiser)
            {
                case SupportedAdvertisers.Admob:
                    ...
                    var tempAdvertiserAdmob =  advertisers.FirstOrDefault(condition => condition.advertiser == SupportedAdvertisers.Admob);
                    if (tempAdvertiserAdmob != null)
                        advertiser = new Advertiser(admobComp, SupportedAdvertisers.Admob, tempAdvertiserAdmob.advertiserSettings);
#endif
                    break;
                case SupportedAdvertisers.AppLovin:
#if AD_TYPE_APPLOVIN
                    var appLovinComp = gameObject.AddComponent<CustomAppLovin>();
                    var tempAdvertiserAppLovin =  advertisers.FirstOrDefault(condition => condition.advertiser == SupportedAdvertisers.AppLovin);
                    if (tempAdvertiserAppLovin != null)
                        advertiser = new Advertiser(appLovinComp, SupportedAdvertisers.AppLovin, tempAdvertiserAppLovin.advertiserSettings);
                    break;
#endif
                default:
        ...
        public void ShowInterstitial(UnityAction InterstitialClosed = null, string Placement = "", bool autoDisableLoading = true)
        {
            if (!enableAds)
            {
            ...
            if (selectedAdvertiser != null)
            {
                ...
                var showSuccess = selectedAdvertiser.ShowInterstitial(InterstitialClosed, Placement, autoDisableLoading);
                //LogEvent
#if PUB_RK //Rocket
        ...
        private void AddPreprocessorDirectiveAdType(string directive, BuildTargetGroup target)
        {
            ...
            {
                case "AD_TYPE_ADMOB":
                    textToWrite = textToWrite.Replace("AD_TYPE_APPLOVIN", "");
                    break;
                case "AD_TYPE_APPLOVIN":
                    textToWrite = textToWrite.Replace("AD_TYPE_ADMOB", "");
                    break;
                default:
